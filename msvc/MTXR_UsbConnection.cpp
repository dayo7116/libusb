...
struct transfer_info {
  ...
  ~transfer_info() { libusb_free_transfer(transfer); }
  ...
  void Notify() {
    //std::cout << "notifying " << name << " transfer complete";
...
class usb_handle {
 public:
  ...
  usb_handle(const std::string& device_address, const std::string& serial,
             unique_device_handle&& device_handle, uint8_t interface,
             uint8_t bulk_in, uint8_t bulk_out, size_t zero_mask,
  ...
  void Close() {
    ...
    // Cancelling transfers will trigger more Closes, so make sure this only
    // happens once.
    if (closing) {
      return;
    }
...
static auto& usb_handles =
    *new std::unordered_map<std::string, std::unique_ptr<usb_handle>>();
...
static bool should_perform_zero_transfer(uint8_t endpoint, size_t write_length,
                                         uint16_t zero_mask) {
...
static void process_device(libusb_device* device) {
  ...
  if (rc != 0) {
    std::cout << "failed to get device descriptor for device at "
                 << device_address << ": " << libusb_error_name(rc);
    return;
  }

  if (device_desc.bDeviceClass != LIBUSB_CLASS_PER_INTERFACE) {
    // Assume that all Android devices have the device class set to per
    // interface.
    // TODO: Is this assumption valid?
    std::cout << "skipping device with incorrect class at "
                 << device_address;
    return;
  }
  ...
  if (rc != 0) {
    std::cout << "failed to get active config descriptor for device at "
                 << device_address << ": " << libusb_error_name(rc);
    return;
  }
  ...
  for (interface_num = 0; interface_num < config->bNumInterfaces;
       ++interface_num) {
    ...
    if (interface.num_altsetting != 1) {
      ...
      // TODO: Is this assumption valid?
      std::cout << "skipping interface with incorrect num_altsetting at "
    ...
    if (!is_adb_interface(interface_desc.bInterfaceClass,
      ...
      std::cout << "skipping non-adb interface at " << device_address
                   << " (interface " << interface_num << ")";
      continue;
    }

    std::cout << "found potential adb interface at " << device_address
                 << " (interface " << interface_num << ")";
    ...
    if (found_in && found_out) {
      ...
      std::cout << "rejecting potential adb interface at " << device_address
                   << "(interface " << interface_num
  ...
  if (!found_adb) {
    std::cout << "skipping device with no adb interfaces at "
                 << device_address;
    return;
  }

  {
    ...
    if (usb_handles.find(device_address) != usb_handles.end()) {
      std::cout << "device at " << device_address
  ...
  if (rc == 0) {
    std::cout << "successfully opened adb device at " << device_address << ", "
               << StringPrintf("bulk_in = %#x, bulk_out = %#x", bulk_in,
                               bulk_out);
    ...
    if (rc == 0) {
      std::cout << "received empty serial from device at " << device_address;
      return;
    } else if (rc < 0) {
      std::cout << "failed to get serial from device at " << device_address
    ...
    if (rc != 0) {
      std::cout << "failed to claim adb interface for device '"
                   << device_serial << "'" << libusb_error_name(rc);
      return;
    }
    ...
    for (uint8_t endpoint : {bulk_in, bulk_out}) {
      ...
      if (rc != 0) {
        std::cout << "failed to clear halt on device '" << device_serial
    ...
    std::cout << "failed to open usb device at " << device_address << ": "
                 << libusb_error_name(rc);
    writable = false;

#if defined(__linux__)
    // libusb doesn't think we should be messing around with devices we don't
    // have write access to, but Linux at least lets us get the serial number
    // anyway.
...
#else
    // On Mac OS and Windows, we're screwed. But I don't think this situation
    // actually happens on those OSes.
    return;
#endif
  }

  std::unique_ptr<usb_handle> result(
      new usb_handle(device_address, device_serial, std::move(handle),
...

  {
   ...
   /* register_usb_transport(usb_handle_raw, device_serial.c_str(),
                           device_address.c_str(), writable);*/
  }
  std::cout << "registered new usb device '" << device_serial << "'";
}
...
#if defined(__linux__)
  // Android's host linux libusb uses netlink instead of udev for device hotplug
  // notification, which means we can get hotplug notifications before udev has
  // updated ownership/perms on the device. Since we're not going to be able to
  // link against the system's libudev any time soon, hack around this by
  // inserting a sleep.
...
static void device_disconnected(libusb_device* device) {
  ...
  std::cout << "device disconnected: " << device_address;
  std::unique_lock<std::mutex> lock(usb_handles_mutex);
...
static auto& hotplug_queue =
    *new BlockingQueue<std::pair<libusb_hotplug_event, libusb_device*>>();
static void hotplug_thread() {
  //adb_thread_setname("libusb hotplug");
  while (true) {
    hotplug_queue.PopAll(
        [](std::pair<libusb_hotplug_event, libusb_device*> pair) {
...
static int hotplug_callback(libusb_context*, libusb_device* device,
                                        libusb_hotplug_event event, void*) {
  // We're called with the libusb lock taken. Call these on a separate thread
  // outside of this function so that the usb_handle mutex is always taken
  // before the libusb mutex.
...
void usb_init() {
  std::cout << "initializing libusb...";
  int rc = libusb_init(nullptr);
  if (rc != 0) {
    std::cout << "failed to initialize libusb: " << libusb_error_name(rc);
  }
  ...
  rc = libusb_hotplug_register_callback(
      ...
      LIBUSB_HOTPLUG_MATCH_ANY, LIBUSB_CLASS_PER_INTERFACE, hotplug_callback,
      nullptr, &hotplug_handle);

  if (rc != LIBUSB_SUCCESS) {
    std::cout << "failed to register libusb hotplug callback";
  }
...
static void transfer_callback(libusb_transfer* transfer) {
  ...
  std::cout << info->name << " transfer callback entered";

  // Make sure that the original submitter has made it to the condition_variable
  // wait.
  std::unique_lock<std::mutex> lock(info->mutex);

  std::cout << info->name << " callback successfully acquired lock";
  ...
  if (info->is_bulk_out && transfer->actual_length != transfer->length) {
    std::cout << info->name << " transfer incomplete, resubmitting";
    transfer->length -= transfer->actual_length;
    transfer->buffer += transfer->actual_length;
    int rc = libusb_submit_transfer(transfer);
    if (rc != 0) {
      std::cout << "failed to submit " << info->name
  ...
  if (should_perform_zero_transfer(transfer->endpoint, transfer->length,
                                   info->zero_mask)) {
    std::cout << "submitting zero-length write";
    transfer->length = 0;
    int rc = libusb_submit_transfer(transfer);
    if (rc != 0) {
      std::cout << "failed to submit zero-length write: "
                   << libusb_error_name(rc);
      transfer->status = LIBUSB_TRANSFER_ERROR;
      info->Notify();
    }
  ...
  std::cout << info->name << "transfer fully complete";
  info->Notify();
}

// Dispatch a libusb transfer, unlock |device_lock|, and then wait for the
// result.
static int perform_usb_transfer(usb_handle* h, transfer_info* info,
  ...
  std::cout << "locking " << info->name << " transfer_info mutex";
  std::unique_lock<std::mutex> lock(info->mutex);
  info->transfer_complete = false;
  std::cout << "submitting " << info->name << " transfer";
  int rc = libusb_submit_transfer(transfer);
  if (rc != 0) {
    std::cout << "failed to submit " << info->name
                 << " transfer: " << libusb_error_name(rc);
  ...
  std::cout << info->name << " transfer successfully submitted";
  device_lock.unlock();
...
int usb_write(usb_handle* h, const void* d, int len) {
  std::cout << "usb_write of length " << len;
...
int usb_read(usb_handle* h, void* d, int len) {
  ...
  int rc = perform_usb_transfer(h, info, std::move(lock));
  std::cout << "usb_read(" << len << ") = " << rc << ", actual_length "
...
int usb_close(usb_handle* h) {
  ...
  if (it == usb_handles.end()) {
    std::cout << "attempted to close unregistered usb_handle for '"
               << h->serial << "'";
  }
...
void usb_kick(usb_handle* h) { h->Close(); }
